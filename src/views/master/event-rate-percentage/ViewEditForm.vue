...
export default {
	...
	created() {
		...
		this.formData.percent = this.formDataSelected.percent;
	...
	setup() {
		...
		// state and getters
		const { data, formDataSelected, saving } = storeToRefs(
			eventRatePercentageStore
		);
		// actions
		const { submitForm } = eventRatePercentageStore;
		// return
		return {
			data,
			formDataSelected,
