<template>
	...
	<div class="w-full">
		<Body>
			<Table label="List of Event Rate Percentage">
				<TableHead>
				...
				>
					...
					>
						...
						<TableData>{{ row.percent }}</TableData>
						<TableData>
...
export default {
	...
	data() {
		return {
			formDataSelected: {},
			tableLabels: ["Title", "Percentage", "Status", "Creator"],
			data: {},
		};
	},
	// methods: {
	...
	setup() {
		...
		const {
			...
			actionResponse,
			forDeletion,
		} = storeToRefs(eventRatePercentageStore);
		// actions
		const {
			getPageLink,
			toggleCreateForm,
